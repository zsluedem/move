...
use crate::diagnostics::codes::{Declarations, Severity, Syntax};
use crate::diagnostics::{Diagnostic, Diagnostics};
use crate::shared::{Identifier, Name, NamedAddressMaps};
use crate::{attr_derivation, diag, CommentMap};
use crate::{shared::CompilationEnv, MatchedFileCommentMap};

use super::comments::Comment;

use super::ast::{self, Visibility, ENTRY_MODIFIER};
use super::cst::{self, SpannedWithComment};
use super::lexer::Tok;
use super::syntax::{make_builtin_call, Modifiers};
        ...
        match tree {
            ...
            cst::ParseTree::Function(function) => {
                return invalid_declaration(
                    ...
                    "'fun' is not allowed in top definition",
                )
            }
            cst::ParseTree::Struct(script) => {
                return invalid_declaration(
                    ...
                    "'struct' is not allowed in top definition",
                )
            }
            cst::ParseTree::Attribute(attrs) => attributes.push(translate_attributes(ast, attrs)?),
            cst::ParseTree::UseDecl(use_) => {
                return invalid_declaration(
                    ast,
                    use_,
                    Declarations::InvalidUse,
                    "'use' is not allowed in top definition",
                )
            }
            cst::ParseTree::FriendDecl(friend) => {
                return invalid_declaration(
                    ...
                    "'friend' is not allowed in top definition",
                )
            }
            cst::ParseTree::Declare(decl) => {
                return invalid_declaration(
                    ...
                    "'let' is not allowed in top definition",
                )
            }
            ...
            cst::ParseTree::Constant(constant) => {
                return invalid_declaration(
                    ...
                    "'const' is not allowed in top definition",
                )
            }
            cst::ParseTree::Exp(exp, _) => {
                return invalid_declaration(
                    ...
                    "expression is not allowed in top definition",
                )
            }
            cst::ParseTree::Spec(spec) => match spec.value().target.value() {
                cst::SpecBlockTarget_::IdentModule(leading, module_name) => {
                    ...
                    let address = Some(translate_token_to_leading_name_access(ast, leading)?);
                    defs.push(ast::PackageDefinition {
                        package: ast.package,
                        named_address_map: ast.named_address_map,
            ...
            cst::ParseTree::SpecMember(member) => {
                return invalid_declaration(
                    ...
                    "spec member is not allowed in top definition",
                )
            }
        }
    }
...
pub fn translate_value(
    ...
    let value = match token.value() {
        cst::Value_::Address(t) => {
            ast::Value_::Address(translate_token_to_leading_name_access(ast, &t)?)
        }
...
pub fn translate_name_access_chain(
    ...
    match &names.value[..] {
        [fir] => Ok(sp(
            loc,
            ast::NameAccessChain_::One(translate_token_to_name(ast, fir)?),
        )),
        [fir, sec] => {
    ...
    for tree in address.value().modules.value() {
        match tree {
            ...
            cst::ParseTree::Attribute(attribute) => {
                internal_attrs.push(translate_attributes(ast, attribute)?)
            }
...
fn translate_attribute_value(
    ...
    match attr_value {
        cst::AttributeValue::Value(v) => {
            ...
            Ok(sp(
                loc,
                ast::AttributeValue_::Value(translate_value(&ast, &v)?),
            ))
        }
...
pub fn translate_constant(
    ...
    let comments = constant.leading_comments(&ast.source_tokens);
...
fn translate_exp_announce(
    ast: &cst::PackageDefinition,
    announce: &cst::Exp,
) -> Result<(Name, Vec<(Name, Vec<ast::Ability>)>, ast::Type), Diagnostic> {
    let loc = announce.loc(&ast.source_tokens);
    match announce.value() {
        cst::Exp_::Announce(name, type_params, res) => {
            if name.value().len() == 1 {
                let name_res = translate_token_to_name(ast, name.value().first().unwrap())?;
                let type_parameters = translate_type_abilities(ast, type_params)?;
                let type_ = translate_type(ast, res)?;
                Ok((name_res, type_parameters, type_))
            } else {
                let name_loc = name.loc(&ast.source_tokens);
                return Err(diag!(
                    Declarations::InvalidName,
                    (name_loc, "declare var could not follow with '::' ")
                ));
            }
        }
        _ => {
            return Err(diag!(
                Declarations::InvalidExpression,
                (loc, "expression is not allowed in 'spec' block")
            ))
        }
    }
}

fn translate_exp_to_spec_member(
    context: &mut Context,
    ast: &cst::PackageDefinition,
    exp: &cst::Exp,
) -> Result<ast::SpecBlockMember, Diagnostic> {
    let loc = exp.loc(&ast.source_tokens);
    let (name, type_parameters, type_, init) = match exp.value() {
        cst::Exp_::Announce(_, _, _) => {
            let (name, type_parameters, type_) = translate_exp_announce(ast, exp)?;
            (name, type_parameters, type_, None)
        }
        cst::Exp_::Assign(lhs, rhs) => {
            let init = translate_exp(context, ast, rhs)?;
            let (name, type_parameters, type_) = translate_exp_announce(ast, exp)?;
            (name, type_parameters, type_, Some(init))
        }
        _ => {
            return Err(diag!(
                Declarations::InvalidExpression,
...
fn translate_exp(
    ...
    let exp_ = match exp.value() {
        cst::Exp_::Value(val) => Ok(ast::Exp_::Value(translate_value(ast, val)?)),
        ...
        cst::Exp_::Announce(_, _, _) => Err(diag!(
            Declarations::InvalidSpecStatement,
            (loc, "Var<Type>: <Type> is only allowed in spec block")
        )),
    }?;
...
fn check_invalid_attributes(
    ...
    if attributes.is_empty() {
        ...
        let loc = loc_between(locs.first().unwrap(), locs.last().unwrap());
        return Err(diag!(
        ...
        match item {
            cst::ParseTree::Declare(decl) => {
                ...
                sequences.push(sp(
                    ...
                    translate_let_declare(ast, decl)?,
                ))
            }
            cst::ParseTree::LetAssign(bind) => {
                check_invalid_attributes(&internal_attrs, "attribute before let is not valid")?;
            ...
            cst::ParseTree::Attribute(attrs) => {
                internal_attrs.push(translate_attributes(ast, attrs)?)
            }
            ...
            cst::ParseTree::Module(s) => {
                return invalid_declaration(
                    ...
                    "'module' is not allowed in expression",
                )
            }
            cst::ParseTree::Script(s) => {
                return invalid_declaration(
                    ...
                    "'script' is not allowed in expression",
                )
            }
            cst::ParseTree::Address(s) => {
                return invalid_declaration(
                    ...
                    s,
                    Declarations::InvalidAddress,
                    "'address' is not allowed in expression",
                )
            }
            cst::ParseTree::Function(s) => {
                return invalid_declaration(
                    ...
                    s,
                    Declarations::InvalidFunction,
                    "'fun' is not allowed in expression",
                )
            }
            cst::ParseTree::Struct(s) => {
                return invalid_declaration(
                    ...
                    s,
                    Declarations::InvalidStruct,
                    "'struct' is not allowed in expression",
                )
            }
            cst::ParseTree::FriendDecl(s) => {
                return invalid_declaration(
                    ...
                    s,
                    Declarations::InvalidFriendDeclaration,
                    "'friend' is not allowed in expression",
                )
            }
            cst::ParseTree::Constant(s) => {
                return invalid_declaration(
                    ...
                    s,
                    Declarations::InvalidConstant,
                    "'const' is not allowed in expression",
                )
            }
            cst::ParseTree::SpecMember(s) => {
                return invalid_declaration(
                    ...
                    s,
                    Declarations::InvalidSpec,
                    "'spec' is not allowed in expression",
                )
            }
        }
    }
...
fn translate_exp_vector(
    ...
    name: &cst::Name,
    type_: &[cst::Type],
    contents: &cst::SpannedWithComment<Vec<cst::Exp>>,
) -> Result<ast::Exp_, Diagnostic> {
    let name_loc = name.loc(&ast.source_tokens);
...
pub fn translate_friend(
    ...
    let friend = translate_name_access_chain(ast, &friend.value())?;
    Ok(ast::FriendDecl {
        attributes,
        loc,
        friend,
...
fn translate_bind(ast: &cst::PackageDefinition, bind: &cst::Bind) -> Result<ast::Bind, Diagnostic> {
    let loc = bind.loc(&ast.source_tokens);
...
fn translate_module_members(
    ...
    for tree in trees.value() {
        match tree {
            ...
            cst::ParseTree::Attribute(attr) => attributes.push(translate_attributes(ast, &attr)?),
            cst::ParseTree::UseDecl(use_) => {
                let attrs = mem::take(&mut attributes);
        ...
        match member {
            cst::ParseTree::Exp(exp, _) => {
                check_invalid_attributes(
                    &internal_attrs,
                    "attribute before expression is not valid",
                )?;
                members.push(translate_exp_to_spec_member(context, ast, exp)?)
            }
...
fn translate_function_to_spec_memeber(
    ...
    let uninterpreted = modifiers
        .iter()
        .find(|m| matches!(m.value(), cst::Modifier_::Native))
        .is_some();
    let func_name = translate_function_name(ast, name)?;
    let signature_res = translate_function_signature(ast, signatures, func_name.loc())?;
...
pub fn translate_struct(
    ...
    let fields = match native {
        Some(loc) => Ok(ast::StructFields::Native(loc)),
        None => {
            let fields_target = struct_content
                ...
                .collect::<Result<Vec<_>, _>>()?;
            Ok(ast::StructFields::Defined(fields_target))
        }
    }?;
...
pub fn translate_type(
    ...
    match type_.value() {
        ...
        cst::Type_::Ability(_, _) => Err(diag!(
            Syntax::InvalidType,
            (loc, "ability type is not allowed here")
        )),
    }
}

fn translate_one_name_access_chain(
    ast: &cst::PackageDefinition,
    chain: &cst::NameAccessChain,
) -> Result<Name, Diagnostic> {
    match &chain.value()[..] {
        [one] => translate_token_to_name(ast, one),
        _ => {
            let loc = chain.loc(&ast.source_tokens);
            Err(diag!(
                Syntax::InvalidNameAccessChain,
                (loc, "name access chain name1::name2.. is not allowed")
            ))
        }
    }
}
...
pub fn translate_type_abilities(
    ...
    type_parameters
        ...
        .map(|t| match t.value() {
            cst::Type_::Apply(names, types) => {
                if types.is_empty() {
                    Ok((translate_one_name_access_chain(ast, names)?, vec![]))
                } else {
                    let loc = t.loc(&ast.source_tokens);
                    Err(diag!(
                        Syntax::InvalidType,
                        (loc, "Only 'type: ability' is allowed")
                    ))
                }
            }
            cst::Type_::Ability(names, a) => {
                let name = translate_one_name_access_chain(ast, names)?;
                let abilities = a
                    .iter()
            ...
            _ => invalid_syntax(
                ...
                t,
                Syntax::InvalidType,
                "only <Type>(:<Ability>*)? is allowed here",
            ),
        })
        .collect::<Result<Vec<_>, _>>()
}
...
pub fn translate_use_decl(
    ...
    let uses_ = translate_use(&ast, &use_.value)?;
    Ok(ast::UseDecl {
        attributes,
        use_: uses_,
    })
...
) -> Result<(ast::Program, CommentMap), Diagnostics> {
...
fn translate_program_(
    ...
    for def in ast.source_definitions.iter() {
        match translate_parsetree_to_def(context, def, &def.source_trees) {
